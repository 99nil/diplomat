...
#readonly caPath=${CA_PATH:-`pwd`/ca}
#readonly caPath=${CA_PATH:-`dirname $0`/ca}
#echo $caPath
readonly caPath=${CA_PATH:-`pwd`/_output/ca}
readonly caSubject=${CA_SUBJECT:-/C=CN/ST=Zhejiang/L=Hangzhou/O=KubeEdge/CN=kubeedge.io}
readonly certPath=${CERT_PATH:-`pwd`/_output/certs}
...
# example: buildSecret -i 127.0.0.1,192.168.1.1
buildSecret() {
    while getopts ':i:h' opt; do
      case $opt in
          i) IPS=($OPTARG)
