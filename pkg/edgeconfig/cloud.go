...
const (
	GroupName  = "cloudcore.diplomat.99nil.com"
	APIVersion = "v1alpha1"
	Kind       = "CloudCore"
)
...
func NewDefaultCloudCoreConfig(rootDir, resourceDir string) *CloudCore {
	...
	cfg := &CloudCore{
		...
		Modules: &CloudModules{
			CloudHub: &CloudHub{
				...
				TLSCAKeyFile:            filepath.Join(rootDir, "/ca/rootCA.key"),
				...
				UnixSocket: &CloudHubUnixSocket{
					Enable:  true,
					Address: "unix://" + filepath.Join(resourceDir, "edge.sock"),
				},
			...
			EdgeController: &EdgeController{
				...
				Buffer: &EdgeControllerBuffer{
					PodEvent:                   DefaultEventBuffer,
					ConfigMapEvent:             DefaultEventBuffer,
					SecretEvent:                DefaultEventBuffer,
					ServiceEvent:               DefaultEventBuffer,
					EndpointsEvent:             DefaultEventBuffer,
					RulesEvent:                 DefaultEventBuffer,
					RuleEndpointsEvent:         DefaultEventBuffer,
					UpdatePodStatus:            DefaultBuffer,
					UpdateNodeStatus:           DefaultBuffer,
					QueryConfigMap:             DefaultBuffer,
					QuerySecret:                DefaultBuffer,
					QueryService:               DefaultBuffer,
					QueryEndpoints:             DefaultBuffer,
					QueryPersistentVolume:      DefaultBuffer,
					QueryPersistentVolumeClaim: DefaultBuffer,
					QueryVolumeAttachment:      DefaultBuffer,
					QueryNode:                  DefaultBuffer,
					UpdateNode:                 DefaultBuffer,
					DeletePod:                  DefaultBuffer,
					ServiceAccountToken:        DefaultBuffer,
				},
			...
			DeviceController: &DeviceController{
				...
				Buffer: &DeviceControllerBuffer{
					UpdateDeviceStatus: DefaultBuffer,
					DeviceEvent:        DefaultEventBuffer,
					DeviceModelEvent:   DefaultEventBuffer,
				},
	...
	cfg.Modules.CloudStream = &CloudStream{
		Enable:                  true,
		StreamPort:              10003,
		TunnelPort:              10004,
		TLSTunnelCAFile:         cfg.Modules.CloudHub.TLSCAFile,
		TLSTunnelCertFile:       cfg.Modules.CloudHub.TLSCertFile,
		TLSTunnelPrivateKeyFile: cfg.Modules.CloudHub.TLSPrivateKeyFile,
		TLSStreamCAFile:         filepath.Join(rootDir, "/ca/streamCA.crt"),
		TLSStreamCertFile:       filepath.Join(rootDir, "/certs/stream.crt"),
		TLSStreamPrivateKeyFile: filepath.Join(rootDir, "/certs/stream.key"),
	}
	return cfg
}

// CloudCore indicates the config of cloudCore which get from cloudCore config file
type CloudCore struct {
	...
	// +Required
	Modules *CloudModules `json:"modules,omitempty"`
...
// CloudModules indicates the modules of CloudCore will be use
...
func (c *CloudCore) Parse(filename string) error {
